 ...
 * @author PierreJean
 */
class DeleteTaskFeature implements Function<String, Observable<Result<ViewModel>>> {
    ...
    public Observable<Result<ViewModel>> apply(String taskId) {
        return Observable.just(taskId)
                ...
                .map(id -> (Result<ViewModel>) DeleteTaskFeature::updateInvalidState)
                .switchIfEmpty(
                        Completable.fromAction(() -> dataSource.deleteTask(taskId))
                                .doOnError(e -> logger.print(getClass(), "Could no delete data", e))
                                .andThen(Observable.just((Result<ViewModel>) DeleteTaskFeature::updateSuccessState))
